...
RSpec.describe User, type: :model do
  ...
  describe 'validationテスト' do
    ...
    context 'nickname' do
      ...
      it '空文字の場合、保存できないこと' do
        user.nickname = ""
        is_expected.to eq(false)
      end

      it '10文字以内の場合、保存できること' do
        user.nickname = Faker::Lorem.unique.characters(number: 10)
        is_expected.to eq(true)
      end

      it '11文字以上の場合、保存できないこと' do
    ...
    context 'email' do
      ...
      it 'emailの形式ではない場合、保存できないこと' do
        user.email = Faker::Lorem.sentence
        is_expected.to eq(false)
      end

      it '全角文字を使用する場合、保存できないこと' do
        user.email = 'ｅｘａｐｌｅ@gmail.com'
        is_expected.to eq(false)
      end

      it 'すでに使用されているemailの場合、保存できないこと' do
        subject
    ...
    context 'password' do
      it 'passwordがnilの場合、保存できないこと' do
        user.password = nil
        is_expected.to eq(false)
      end

      it 'passwordが空文字の場合、保存できないこと' do
        user.password = ""
        is_expected.to eq(false)
      end

      it 'password_confirmationがnilの場合、保存できないこと' do
        user.password_confirmation = nil
        is_expected.to eq(false)
      end

      it 'password_confirmationが空文字の場合、保存できないこと' do
        user.password_confirmation = ""
        is_expected.to eq(false)
      end

      it 'passwordとpassword_confirmationが不一致の場合、保存できないこと' do
        user.password = 'password'
        user.password_confirmation = 'password_confirmation'
        is_expected.to eq(false)
      end

      it '5文字以下の場合、保存できないこと' do
        user.password = user.password_confirmation = Faker::Lorem.characters(number: 5)
        is_expected.to eq(false)
      end

      it '6文字以上、128文字以下の場合、保存できること' do
